...
def apply() -> None:
    """Apply all CMSS monkey-patches targeting vllm-ascend."""
    global _APPLIED
    if _APPLIED:
    ...
    for suffix in _PATCH_MODULES:
        patch = import_module(f"vllm_ascend.patch_cmss.vllm_ascend.{suffix}")
    ...
    for suffix in (
        "distributed.kv_transfer.kv_p2p.mooncake_connector",
        "distributed.kv_transfer.kv_p2p.mooncake_hybrid_connector",
    ):
        patch = import_module(f"vllm_ascend.patch_cmss.vllm_ascend.{suffix}")
        patch.apply()
