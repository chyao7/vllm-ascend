...
#include "../../mla_preprocess/op_kernel/kernel/mem.h"
#include "../../mla_preprocess/op_kernel/kernel/mma.h"
#include "../../mla_preprocess/op_kernel/kernel/utils.h"
#include "../../mla_preprocess/op_kernel/kernel/simd.h"
#include "../../mla_preprocess/op_kernel/kernel/kernel_utils.h"

#include "lib/matmul_intf.h"

#include "mla_preprocess_merged_dtile.h"
#include "mla_preprocess_merged_dtile_quant.h"
#include "../op_host/tiling/mla_preprocess_merged_dtile_tiling.h"
...
class MLAOperation
{
    ...
    using K_NOPE_DTYPE =
        typename std::conditional_t<CACHE_MODE == CACHE_MODE_INT8_NZCACHE || MERGED_DTILE_BUILD,
                                      int8_t, InDtype>;
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR hiddenStateGm, GM_ADDR quantScale1Gm,
                                ...
                                GM_ADDR gmKNopeClipAlpha, GM_ADDR qGm, GM_ADDR keycacheOutGm, GM_ADDR qGm2,
                                GM_ADDR keycacheOutGm2, GM_ADDR s1Gm, GM_ADDR s2Gm, GM_ADDR s3Gm, GM_ADDR s4Gm,
                                GM_ADDR s5Gm, GM_ADDR innerGm)
    {
        quantScale3GmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ InDtype *>(gmCtkvScale));
        kNopeClipAlphaGmTensor.SetGlobalBuffer(reinterpret_cast<__gm__ float *>(gmKNopeClipAlpha));
        ...
        debugTraceGm_.SetGlobalBuffer(reinterpret_cast<__gm__ int32_t *>(keycacheOutGm2));
...
private:
    ...
    __aicore__ inline void RmsNormAndRopeConvergence1(
        ...
        AscendC::LocalTensor<float> &rmsNormTensor, const AscendC::LocalTensor<float> &gammaFp32,
        ...
        AscendC::LocalTensor<half> &tmpfp16, AscendC::LocalTensor<int8_t> &int8OutTensor,
        const AscendC::LocalTensor<uint8_t> &quantShareTmpUb, float quantScale3, float kNopeClipAlpha)
    {
        MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_K_LOOP, sN, vectorBlockIdx);
        ...
        for (uint64_t loop = 0; loop < sN; ++loop) {
            ...
            AscendC::PipeBarrier<PIPE_V>();
            MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_RMS, loop, slotValue);
            if constexpr (CACHE_MODE == CACHE_MODE_INT8_NZCACHE) {
                // quant
                ...
                AscendC::PipeBarrier<PIPE_V>();
            } else if constexpr (MERGED_DTILE_BUILD) {
#if MERGED_DTILE_ENABLE_PER_TILE_QUANT
                // Per-tile quant; scratch in UB gap (no tmp32/calTensor alias).
                PerTileQuantKNormProlog(rmsNormTensor, int8OutTensor, quantShareTmpUb, splitRmsNormSizeOne_,
                                        KV_PER_TILE_QUANT_SIZE, kNopeClipAlpha, debugTraceGm_);
                MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_QUANT, loop, 1);
#else
                CastFrom32To16(tmpfp16, rmsNormTensor, splitRmsNormSizeOne_);
                AscendC::PipeBarrier<PIPE_V>();
                CastFromF16ToI8(int8OutTensor, tmpfp16, -128, splitRmsNormSizeOne_);
                AscendC::PipeBarrier<PIPE_V>();
                MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_QUANT, loop, 0);
#endif
            } else {
                AscendC::PipeBarrier<PIPE_V>();
            ...
            AscendC::PipeBarrier<PIPE_V>();
            MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_ROPE, loop, slotValue);
            if constexpr (MERGED_DTILE_BUILD) {
                // Merged dtile: k_pe bf16 at byte MERGED_DTILE_KPE_BYTE_OFFSET in the staging row.
                if (std::is_same<T1, __bf16>::value) {
                    Cast(outTmpTensor[MERGED_DTILE_KPE_BYTE_OFFSET / sizeof(T1)], ropeKRevertTensor,
                         AscendC::RoundMode::CAST_RINT, splitRmsNormSizeTwo_);
                } else {
                    Cast(outTmpTensor[MERGED_DTILE_KPE_BYTE_OFFSET / sizeof(T1)], ropeKRevertTensor,
                         AscendC::RoundMode::CAST_NONE, splitRmsNormSizeTwo_);
                }
            } else if (std::is_same<T1, __bf16>::value) {
            ...
            if constexpr (CACHE_MODE == CACHE_MODE_KVCACHE) {
...
#if MERGED_DTILE_ENABLE_KV_SCATTER
                // mla_prolog_v3 ckvkr_repo_mode=1 + quant_scale_repo_mode=1 ND scatter (stride=dtileSize).
                const uint64_t cacheBase =
                    static_cast<uint64_t>(slotValue) * static_cast<uint64_t>(MERGED_DTILE_BYTES);
                AscendC::DataCopyExtParams outExt;
                outExt.blockCount = 1;
                outExt.srcStride = 0;
                outExt.dstStride = 0;
                // Scatter ckv (k_nope int8) -> kv_cache[token][0 : headSizeCkv).
                outExt.blockLen = MERGED_DTILE_KNOPE_BYTES;
                DataCopyPad(keycacheGmTensor1[cacheBase], int8OutTensor, outExt);
                // Scatter kr (k_pe bf16) -> kv_cache[token][headSizeCkv : headSizeCkv + dr*2).
                outExt.blockLen = MERGED_DTILE_KPE_BYTE_LEN;
                DataCopyPad(keycacheGmTensor1[cacheBase + MERGED_DTILE_KPE_BYTE_OFFSET],
                            int8OutTensor[MERGED_DTILE_KPE_BYTE_OFFSET], outExt);
                // Scatter per-tile descale -> kv_cache[token][headSizeCkv + dr*2 : dtileSize).
                outExt.blockLen = MERGED_DTILE_SCALE_BYTES;
                DataCopyPad(keycacheGmTensor1[cacheBase + MERGED_DTILE_SCALE_BYTE_OFFSET],
                            int8OutTensor[MERGED_DTILE_SCALE_BYTE_OFFSET], outExt);
                MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_SCATTER, loop, slotValue);
#elif MERGED_DTILE_ENABLE_ROW_BUFFER
                // Contiguous [N, dtileSize] row buffer: token row index, no paged scatter.
                const uint64_t rowIdx = static_cast<uint64_t>(row_work * vectorBlockIdx + loop);
                const uint64_t cacheBase = rowIdx * static_cast<uint64_t>(MERGED_DTILE_BYTES);
                AscendC::DataCopyExtParams outExt;
                outExt.blockCount = 1;
                outExt.srcStride = 0;
                outExt.dstStride = 0;
                ...
                DataCopyPad(keycacheGmTensor1[cacheBase + MERGED_DTILE_SCALE_BYTE_OFFSET],
                            int8OutTensor[MERGED_DTILE_SCALE_BYTE_OFFSET], outExt);
                MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_ROW_WRITE, loop,
...
private:
    ...
    AscendC::GlobalTensor<float> kNopeClipAlphaGmTensor;
    ...
    AscendC::GlobalTensor<int32_t> debugTraceGm_;
...
MLAOperation<InDtype, CACHE_MODE, weightFormat1, weightFormat2, weightFormat3, quantMode>::ProcessVector()
{
    ...
    // K RMS/RoPE/quant/scatter before mm2 AIV: same order as mla_preprocess_mix_bf16_qdown (CACHE_MODE_KROPE_CTKV).
    if (row_work_ != 0) {
        AscendC::LocalTensor<InDtype> input_tensor = buf.GetBuffer<BufferType::ASCEND_UB, InDtype>(0);
        AscendC::LocalTensor<InDtype> gamma_tensor = buf.GetBuffer<BufferType::ASCEND_UB, InDtype>(mm1OutSize_ * 2);
        ...
        int32_t tmp32_block_bytes = static_cast<int32_t>(splitRmsNormSizeOne_ * 3 * 4 + splitRmsNormSizeTwo_ * 2 * 4 +
                                                           mm1OutSize_ * 4 * 2 + 32);
        int32_t out_ub_offset = mm1OutSize_ * 2 + splitRmsNormSizeOne_ * 2 + splitRmsNormSizeTwo_ * 4 + 4096 * 32 +
                                tmp32_block_bytes;

        AscendC::LocalTensor<half> tmpfp16;
        AscendC::LocalTensor<int8_t> int8OutTensor;
        AscendC::LocalTensor<uint8_t> quantShareTmpUb;
        AscendC::LocalTensor<InDtype> temp_tensor;
        float scale3 = 0;
        if constexpr (CACHE_MODE == CACHE_MODE_INT8_NZCACHE || MERGED_DTILE_BUILD) {
            // Same UB layout as mla_preprocess_mix_bf16_qdown ProcessVector (INT8_NZ).
            ...
            if constexpr (MERGED_DTILE_BUILD) {
                // Quant scratch after merged-row staging (out_ub); avoids rmsNormQuant2 / tmp32 overlap.
                const int32_t quant_scratch_ub_offset =
                    (out_ub_offset + static_cast<int32_t>(MERGED_DTILE_BYTES) +
                     static_cast<int32_t>(MERGED_DTILE_ALIGN32) - 1) /
                    static_cast<int32_t>(MERGED_DTILE_ALIGN32) * static_cast<int32_t>(MERGED_DTILE_ALIGN32);
                quantShareTmpUb = buf.GetBuffer<BufferType::ASCEND_UB, uint8_t>(quant_scratch_ub_offset);
                tmpfp16 = quantShareTmpUb[MERGED_SCRATCH_TMPFP16_BYTE_OFFSET].template ReinterpretCast<half>();
                int8OutTensor = buf.GetBuffer<BufferType::ASCEND_UB, int8_t>(out_ub_offset);
                temp_tensor = buf.GetBuffer<BufferType::ASCEND_UB, InDtype>(out_ub_offset);
            } else {
                int8OutTensor = buf.GetBuffer<BufferType::ASCEND_UB, int8_t>(out_ub_offset);
                temp_tensor = buf.GetBuffer<BufferType::ASCEND_UB, InDtype>(out_ub_offset);
                tmpfp16 = buf.GetBuffer<BufferType::ASCEND_UB, half>(rms3_ub_offset +
                                                                     splitRmsNormSizeOne_ * sizeof(float) * 2);
                quantShareTmpUb = buf.GetBuffer<BufferType::ASCEND_UB, uint8_t>(rms3_ub_offset);
            }
            if constexpr (CACHE_MODE == CACHE_MODE_INT8_NZCACHE) {
        ...
        float kNopeClipAlpha = 1.0f;
        if constexpr (MERGED_DTILE_BUILD) {
            kNopeClipAlpha = kNopeClipAlphaGmTensor.GetValue(0);
        }

#if MERGED_DTILE_ENABLE_K_RMSROPE
        MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_PROCESS_VECTOR_K, row_work_, 0);
        MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_BEFORE_K_RMSROPE, row_work_, rms3_ub_offset);
        RmsNormAndRopeConvergence1<InDtype>(
            ...
            tmp32_tensor[splitRmsNormSizeOne_ + splitRmsNormSizeOne_ + splitRmsNormSizeTwo_ +
                         splitRmsNormSizeTwo_],
            temp_tensor, tmpfp16, int8OutTensor, quantShareTmpUb, scale3, kNopeClipAlpha);
        MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_K_RMSROPE, row_work_, out_ub_offset);
#endif
    }

    MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_BEFORE_MM2_AIV, row_work_, 0);
    mm_w8a8_aiv_2.Process();
    FftsCrossCoreSync<PIPE_MTE3, 0>(MM2OUT);
    WaitFlagDev(MM2OUT);
    AscendC::CrossCoreSetFlag<0x2, PIPE_MTE3>(AIC_MM3_START);
    MergedDtileDebugTrace(debugTraceGm_, MERGED_DTILE_DBG_AFTER_MM2_AIV, row_work_, 0);
