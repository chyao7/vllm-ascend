...
// Row-buffer variant: same MLAPO merged Dtile compute as mla_preprocess_merged_dtile,
// but writes [N, 656] contiguous dtile rows instead of paged-cache scatter.
#undef MERGED_DTILE_ENABLE_KV_SCATTER
#undef MERGED_DTILE_ENABLE_ROW_BUFFER
#define MERGED_DTILE_ENABLE_KV_SCATTER 0
#define MERGED_DTILE_ENABLE_ROW_BUFFER 1

#include "kernel_operator.h"
#include "../../kernels/types.h"

#include "mla_preprocess_merged_dtile.h"
#include "mla_preprocess_merged_dtile_inner.hpp"
#include "../op_host/tiling/mla_preprocess_merged_dtile_tiling.h"

extern "C" __global__ __aicore__ void mla_preprocess_merged_dtile_rows(
    ...
    GM_ADDR bias2, GM_ADDR wuk, GM_ADDR descale1, GM_ADDR descale2, GM_ADDR ctkvScale, GM_ADDR qnopeScale,
    GM_ADDR kNopeClipAlpha, GM_ADDR q, GM_ADDR dtileRowsOut, GM_ADDR q2, GM_ADDR keycacheOut2, GM_ADDR innerOut,
    GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    MLAPO_MERGED_DTILE::MLAOperation<__bf16, CACHE_MODE_KROPE_CTKV, DataFormat::NZ, DataFormat::NZ, DataFormat::ND,
                                     QuantMode::PER_TENSOR_ASYMM_QUANT>
        opMergedDtile(mlaTilingData, tiling);
    opMergedDtile.Init(hiddenState, quantScale1, quantOffset1, wdqkv, bias1, gamma2, beta2, quantScale2, quantOffset2,
                       gamma3, sin1, cos1, sin2, cos2, keycache, slotMapping, wuq, bias2, wuk, descale1, descale2,
                       ctkvScale, qnopeScale, kNopeClipAlpha, q, dtileRowsOut, q2, keycacheOut2, s1, s2, s3, s4, s5,
                       innerOut);
    if ASCEND_IS_AIC {
        opMergedDtile.ProcessCube();
    }
    if ASCEND_IS_AIV {
        opMergedDtile.ProcessVector();
    }
}

namespace vllm_ascend {

extern void mla_preprocess_merged_dtile_rows_impl(
    void* stream,
    void* hidden_state,
    void* quant_scale1,
    void* quant_offset1,
...
{
    mla_preprocess_merged_dtile_rows<<<block_dim, nullptr, stream>>>(
        hidden_state, quant_scale1, quant_offset1, wdqkv, bias1, gamma2, beta2, quant_scale2, quant_offset2, gamma3,
        sin1, cos1, sin2, cos2, keycache, slot_mapping, wuq, bias2, wuk, descale1, descale2, ctkv_scale, qnope_scale,
        k_nope_clip_alpha, q, dtile_rows_out, q2, keycache_out2, inner_out, workspace, tiling);
}
