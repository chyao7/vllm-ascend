 ...
 */
#ifndef MLA_PREPROCESS_MERGED_DTILE_TORCH_ADPT_H
#define MLA_PREPROCESS_MERGED_DTILE_TORCH_ADPT_H
...
inline const char *MergedDtileDebugStageName(int32_t stage)
{
    switch (stage) {
        case 100: return "process_vector_k";
        case 110: return "before_k_rmsrope";
        case 120: return "after_k_rmsrope";
        case 130: return "before_mm2_aiv";
        case 131: return "after_mm2_aiv";
        case 200: return "k_loop";
        case 220: return "after_k_rms";
        case 230: return "after_k_quant";
        case 240: return "after_k_rope";
        case 250: return "after_k_scatter";
        case 251: return "after_k_row_write";
        case 400: return "quant_start";
        case 401: return "quant_tile";
        case 409: return "quant_done";
        default: return "unknown";
    }
}

inline void PrintMergedDtileDebugTrace(const at::Tensor &debug_trace, const char *where)
{
    if (debug_trace.numel() < 4) {
        std::fprintf(stderr, "[merged_dtile][%s] debug_trace too small (need >=4 int32)\n", where);
        return;
    }
    auto cpu = debug_trace.reshape({-1}).to(at::kCPU);
    auto acc = cpu.accessor<int32_t, 1>();
    const int32_t stage = acc[0];
    const int32_t aux0 = acc[1];
    const int32_t aux1 = acc[2];
    const int32_t block = acc[3];
    std::fprintf(stderr,
                 "[merged_dtile][%s] stage=%d(%s) aux0=%d aux1=%d block=%d\n",
                 where, stage, MergedDtileDebugStageName(stage), aux0, aux1, block);
}

inline bool MergedDtileDebugEnabled()
{
    const char *env = std::getenv("VLLM_ASCEND_MERGED_DTILE_DEBUG");
...
std::tuple<at::Tensor &, at::Tensor &, at::Tensor &, at::Tensor &> mla_preprocess_merged_dtile(
    ...
    const at::Tensor &wuk, const at::Tensor &kv_cache, const at::Tensor &slotmapping,
    ...
    at::Tensor &q_out0, at::Tensor &kv_cache_out, at::Tensor &q_out1, at::Tensor &inner_out)
{
    at::Tensor Descale0 =
        descale0.has_value()
            ? descale0.value()
    ...
    void *kv_cache_out_ptr = kv_cache_out.data_ptr();
    void *q_out1_ptr = q_out1.data_ptr();
    void *inner_out_ptr = inner_out.data_ptr();
    // keycacheOutGm2 carries optional GM debug trace (int32[0..3]) for MERGED_DTILE.
    ...
    cmd.Name("mla_preprocess_merged_dtile");

    cmd.SetCustomHandler([stream, hidden_state_ptr, quant_scale0_ptr, quant_offset0_ptr, wdqkv_ptr, bias0_ptr, gamma1_ptr,
                          beta1_ptr, quant_scale1_ptr, quant_offset1_ptr, gamma2_ptr, sin_ptr, cos_ptr, kv_cache_ptr,
                          slotmapping_ptr, wuq_ptr, bias1_ptr, wuk_ptr, descale0_ptr, descale1_ptr, ctkv_scale_ptr,
                          qnope_scale_ptr, k_nope_clip_alpha_ptr, q_out0_ptr, kv_cache_out_ptr, q_out1_ptr,
                          k_pe_scratch_ptr, inner_out_ptr, workspace_ptr, tiling_ptr, block_dim]() -> int {
        mla_preprocess_merged_dtile_impl(
            stream, hidden_state_ptr, quant_scale0_ptr, quant_offset0_ptr, wdqkv_ptr, bias0_ptr, gamma1_ptr, beta1_ptr,
            quant_scale1_ptr, quant_offset1_ptr, gamma2_ptr, sin_ptr, cos_ptr, sin_ptr, cos_ptr, kv_cache_ptr,
            slotmapping_ptr, wuq_ptr, bias1_ptr, wuk_ptr, descale0_ptr, descale1_ptr, ctkv_scale_ptr, qnope_scale_ptr,
            k_nope_clip_alpha_ptr, q_out0_ptr, kv_cache_out_ptr, q_out1_ptr, k_pe_scratch_ptr, inner_out_ptr,
            workspace_ptr, tiling_ptr, block_dim);
        return 0;
    });
    cmd.Run();
    if (MergedDtileDebugEnabled()) {
        PrintMergedDtileDebugTrace(k_pe_scratch.reshape({-1}), "host_after_run");
    }
    return std::forward_as_tuple(q_out0, kv_cache_out, q_out1, inner_out);
}
}
#endif
