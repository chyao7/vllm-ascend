# aclnnInt8SparseFlashAttention

[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/master/attention/int8_sparse_flash_attention)
...
- 接口功能：int8_sparse_flash_attention（SFA）是针对大序列长度推理场景的高效注意力计算模块，该模块通过“只计算关键部分”大幅减少计算量，然而会引入大量的离散访存，造成数据搬运时间增加，进而影响整体性能。
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnInt8SparseFlashAttentionGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnInt8SparseFlashAttention”接口执行计算。
...
```Cpp
aclnnStatus aclnnInt8SparseFlashAttention(
    void             *workspace, 
    uint64_t          workspaceSize, 
    aclOpExecutor    *executor, 
...
## aclnnInt8SparseFlashAttentionGetWorkspaceSize
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnInt8SparseFlashAttentionGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：aclnnInt8SparseFlashAttention默认确定性实现。
- 该接口支持推理场景下使用。
- N1支持1~64和128。
...
#include "securec.h"
#include "acl/acl.h"
#include "aclnnop/aclnn_int8_sparse_flash_attention.h"
...
int ExecuteInt8SparseFlashAttention(TensorResources& resources, aclrtStream stream, 
    ...
    int ret = aclnnInt8SparseFlashAttentionGetWorkspaceSize(resources.queryTensor, resources.keyTensor, resources.valueTensor, resources.sparseIndicesTensor, nullptr, nullptr, nullptr, resources.queryRopeTensor, resources.keyRopeTensor,
    ...
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnInt8SparseFlashAttentionGetWorkspaceSize failed. ERROR: %d\n", ret);
        return ret;
    }
    ...
    ret = aclnnInt8SparseFlashAttention(*workspaceAddr, *workspaceSize, executor, stream);
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnInt8SparseFlashAttention failed. ERROR: %d\n", ret);
        return ret;
    }
...
int main() {
    ...
    ret = ExecuteInt8SparseFlashAttention(resources, stream, &workspaceAddr, &workspaceSize);
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        CleanupResources(resources, workspaceAddr, stream, deviceId);
        return ret;
    }
