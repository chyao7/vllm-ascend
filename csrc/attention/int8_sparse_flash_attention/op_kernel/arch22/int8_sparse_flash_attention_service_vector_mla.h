...
/*!
 * \file int8_sparse_flash_attention_service_vector_mla.h
 * \brief
 */
#ifndef INT8_SPARSE_FLASH_ATTENTION_SERVICE_VECTOR_MLA_H
#define INT8_SPARSE_FLASH_ATTENTION_SERVICE_VECTOR_MLA_H
...
public:
    ...
    __aicore__ inline void InitParams(const struct ConstInfo &constInfo,
                                      const Int8SparseFlashAttentionTilingDataMla *__restrict tilingData);
    ...
    __aicore__ inline void InitVec0GlobalTensor(const GlobalTensor<int32_t> &kvValidSizeGm,
                                                ...
                                                const GlobalTensor<KV_INT8_T> &keyGm,
    ...
    __aicore__ inline void DequantInt8Row(LocalTensor<KV_T> &dstUb, LocalTensor<KV_INT8_T> srcInt8Ub, float scale,
                                          float offset, uint32_t headDim);
    ...
    static constexpr uint32_t KV_MERGE_STAGE_MAX_DIM = 528U;
    ...
    LocalTensor<KV_INT8_T> kvMergUb_;
    LocalTensor<KV_T> ropeMergUb_;
    LocalTensor<KV_T> dequantTmpUb_;
...
template <typename SFAT> __aicore__ inline void SFAVectorService<SFAT>::InitBuffers(TPipe *pipe)
{
    ...
    kvMergUb_ = inputBuff1.Get<KV_INT8_T>();
    ropeMergUb_ = inputBuff2.Get<KV_T>();
    dequantTmpUb_ = outputBuff2.Get<KV_T>();
...
__aicore__ inline void SFAVectorService<SFAT>::InitVec0GlobalTensor(
    ...
    const GlobalTensor<KV_T> &keyRopeGm, const GlobalTensor<KV_INT8_T> &keyGm,
    const GlobalTensor<int32_t> &blkTableGm)
{
...
template <typename SFAT>
__aicore__ inline void SFAVectorService<SFAT>::DequantInt8Row(LocalTensor<KV_T> &dstUb,
                                                              LocalTensor<KV_INT8_T> srcInt8Ub, float scale,
                                                              float offset, uint32_t headDim)
{
    // arch22 supports int8->half and half->float, but not int8->float directly.
    LocalTensor<half> halfUb = tmpBuff1.Get<half>();
    LocalTensor<float> fp32Ub = tmpBuff1.Get<float>()[headDim];
    Cast(halfUb, srcInt8Ub, RoundMode::CAST_NONE, headDim);
    Cast(fp32Ub, halfUb, RoundMode::CAST_NONE, headDim);
    Muls(fp32Ub, fp32Ub, scale, headDim);
    Adds(fp32Ub, fp32Ub, offset, headDim);
    Cast(dstUb, fp32Ub, RoundMode::CAST_ROUND, headDim);
}
                                                                    ...
                                                                    uint32_t headDim)
{
    ...
    for (uint32_t tileIdx = 0; tileIdx < KV_TILE_NUM; ++tileIdx) {
        ...
        LocalTensor<KV_INT8_T> tileInt8Ub = packedRowUb[start];
                                       ...
                                       int64_t keyBNBOffset,int64_t s2IdLimit, const RunInfo &runInfo)
{
    ...
    const uint64_t stageRowStride = constInfo.keyQuantMode != 0 ? constInfo.kvStorageDim : constInfo.headDim;
                                                                 ...
                                                                 const RunInfo &runInfo)
{
    ...
    const float scale = constInfo.kvDequantScale;
    const float offset = constInfo.kvDequantOffset;
    const uint64_t stageRowStride = constInfo.keyQuantMode != 0 ? constInfo.kvStorageDim : constInfo.headDim;
    for (int64_t rowIdx = 0; rowIdx < rowCount; ++rowIdx) {
        ...
        if (constInfo.keyQuantMode != 0) {
            DequantInt8RowPerTile(dequantTmpUb_, srcInt8RowUb, constInfo.headDim);
        } else {
            DequantInt8Row(dequantTmpUb_, srcInt8RowUb, scale, offset, constInfo.headDim);
        }
...
__aicore__ inline void SFAVectorService<SFAT>::MergeKv(const RunInfo &runInfo)
{
    ...
    if (unlikely(s2GmStartOffset + mte2Size < s2GmLimit)) {
        ...
        Duplicate(dequantTmpUb_, static_cast<KV_T>(0.0), constInfo.headDim);
        ...
        for (int64_t s2GmOffset = s2GmStartOffset + mte2Size; s2GmOffset < s2GmLimit; s2GmOffset++) {
            DataCopyPad(kvMergeGm_[runInfo.loop % MERGE_CACHE_GM_BUF_NUM * 512 * 576 + s2GmOffset * constInfo.headDim],
                        dequantTmpUb_, dataCopyParams);
        }
        Duplicate(ropeMergUb_, static_cast<KV_T>(0.0), constInfo.headDimRope);
        SetFlag<AscendC::HardEvent::V_MTE3>(0);
        WaitFlag<AscendC::HardEvent::V_MTE3>(0);
        dataCopyParams.blockLen = constInfo.headDimRope * sizeof(KV_T);
        for (int64_t s2GmOffset = s2GmStartOffset + mte2Size; s2GmOffset < s2GmLimit; s2GmOffset++) {
...
#endif // INT8_SPARSE_FLASH_ATTENTION_SERVICE_VECTOR_MLA_H
