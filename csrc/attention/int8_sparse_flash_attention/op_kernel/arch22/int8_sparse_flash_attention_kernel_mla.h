...
/*!
 * \file int8_sparse_flash_attention_kernel_mla.h
 * \brief
 */

#ifndef INT8_SPARSE_FLASH_ATTENTION_KERNEL_MLA_H
#define INT8_SPARSE_FLASH_ATTENTION_KERNEL_MLA_H
...
#include "../int8_sparse_flash_attention_common.h"
#include "int8_sparse_flash_attention_service_cube_mla.h"
#include "int8_sparse_flash_attention_service_vector_mla.h"
...
template <typename SFAT> class Int8SparseFlashAttentionMla {
public:
    // 中间计算数据类型为float，高精度模式
    using T = float;
    using Q_T = typename SFAT::queryType;
    using KV_INT8_T = typename SFAT::kvInt8Type;
    ...
    __aicore__ inline Int8SparseFlashAttentionMla(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                __gm__ uint8_t *workspace, const Int8SparseFlashAttentionTilingDataMla *__restrict tiling,
...
private:
    ...
    const Int8SparseFlashAttentionTilingDataMla *__restrict tilingData = nullptr;
    ...
    GlobalTensor<KV_INT8_T> keyGm;
    GlobalTensor<KV_INT8_T> valueGm;
...
template <typename SFAT> __aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::InitTilingData()
{
...
template <typename SFAT> __aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
Int8SparseFlashAttentionMla<SFAT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::InitAllZeroOutput(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::InitOutputSingleCore()
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::GetActualSeqLen(uint32_t bIdx, uint32_t s1Idx)
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::GetSparseActualSeqLen(uint32_t bIdx, uint32_t s1Idx,
                                                                            uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline uint32_t Int8SparseFlashAttentionMla<SFAT>::GetActualSeqLenKV(uint32_t bIdx)
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
    if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::GetPreNextTokensLeftUp()
{
    if (constInfo.sparseMode == 3) {
...
template <typename SFAT> __aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::UpdateInnerLoopCond()
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::UpdateInner(uint32_t &s2End, uint32_t &curS2End,
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::Init(__gm__ uint8_t *query,
                       ...
                       __gm__ uint8_t *gmTiling, TPipe *tPipe)
{
    ...
    keyGm.SetGlobalBuffer((__gm__ KV_INT8_T *)keyPtr);
    valueGm.SetGlobalBuffer((__gm__ KV_INT8_T *)valuePtr);
    ...
    if ASCEND_IS_AIC {
        ...
        GlobalTensor<KV_T> keyGmDequantStub;
        GlobalTensor<KV_T> valueGmDequantStub;
        matmulService.InitMm1GlobalTensor(queryGm, qRopeGm, keyGmDequantStub, kRopeGm, mm1ResGm);
        matmulService.InitMm2GlobalTensor(vec1ResGm, valueGmDequantStub, mm2ResGm, attentionOutGm);
...
template <typename SFAT> __aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::InitCalcParamsEach()
{
    //计算总的基本块
...
template <typename SFAT>
__aicore__ inline void
Int8SparseFlashAttentionMla<SFAT>::Bmm2DataCopyOut(uint64_t attenOutOffset, LocalTensor<OUT_T> &attenOutUb,
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::CalcParams(uint32_t loop, uint64_t s2Start,
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::ComputeMm1(const RunInfo &info)
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::ComputeMm2(const RunInfo &info)
{
    uint32_t nBufferLoopTimes = (info.actMBaseSize + constInfo.nBufferMBaseSize - 1) / constInfo.nBufferMBaseSize;
    uint32_t nBufferTail = info.actMBaseSize - (nBufferLoopTimes - 1) * constInfo.nBufferMBaseSize;
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
...
template <typename SFAT> __aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::Process()
{
    if (aiCoreIdx < usedCoreNum) {
        if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::GetBN2Idx(uint32_t bN2Idx, uint32_t &bIdx,
                                                                                uint32_t &n2Idx)
{
...
template <typename SFAT> __aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::ProcessBalance()
{
...
template <typename SFAT>
__aicore__ inline void
Int8SparseFlashAttentionMla<SFAT>::PreloadPipeline(uint32_t loop, uint64_t s2Start, uint64_t s2LoopIdx,
                                                               RunInfo extraInfo[SFA_PRELOAD_TASK_CACHE_SIZE], uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
{
...
Int8SparseFlashAttentionMla<SFAT>::GetBalanceActualSeqLengths(GlobalTensor<int32_t> &actualSeqLengths,
                                                                          uint32_t bIdx)
{
    if constexpr (LAYOUT_T == SFA_LAYOUT::TND) {
        if (bIdx > 0) {
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                      uint32_t s1GEndPrev,
                                                                                      uint32_t s2EndPrev)
{
...
template <typename SFAT>
__aicore__ inline void Int8SparseFlashAttentionMla<SFAT>::CalcSinnerTopKBegin(RunInfo &info, uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
...
#endif // INT8_SPARSE_FLASH_ATTENTION_KERNEL_MLA_H
