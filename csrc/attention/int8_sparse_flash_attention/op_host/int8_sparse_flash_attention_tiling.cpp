...
/*!
 * \file int8_sparse_flash_attention_tiling.cpp
 * \brief
 */
...
#include "../op_kernel/int8_sparse_flash_attention_template_tiling_key.h"
#include "int8_sparse_flash_attention_tiling.h"
...
struct Int8SparseFlashAttentionCompileInfo {
    int64_t core_num;
};

} // namespace optiling

namespace optiling::int8_sfa {
...
static std::string SFADataTypeToSerialString(ge::DataType type)
{
    ...
    if (it != DATATYPE_TO_STRING_MAP.end()) {
        ...
        OP_LOGE("Int8SparseFlashAttention", "datatype %d not support", type);
        return "UNDEFINED";
    }
}
...
ge::graphStatus SFAInfoParser::GetOpName()
{
    if (context_->GetNodeName() == nullptr) {
        OP_LOGE("Int8SparseFlashAttention", "opName got from TilingContext is nullptr");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SFAInfoParser::GetAttrParaInfo()
{
    ...
    opParamInfo_.kvDequantScale = attrs->GetAttrPointer<float>(KEY_SCALE_ATTR_INDEX);
    opParamInfo_.kvDequantOffset = attrs->GetAttrPointer<float>(KEY_OFFSET_ATTR_INDEX);
    return ge::GRAPH_SUCCESS;
}
...
{
    if (context_ == nullptr) {
        OP_LOGE("Int8SparseFlashAttention", "tiling context is nullptr!");
        return ge::GRAPH_FAILED;
    }
    ...
    if (
        ...
        ge::GRAPH_SUCCESS != GetKvStorageDim() ||
...
ge::graphStatus TilingInt8SparseFlashAttention(gert::TilingContext *context)
{
    ::optiling::int8_sfa::SFATilingInfo sfaInfo;
    ::optiling::int8_sfa::SFAInfoParser sfaInfoParser(context);
    if (sfaInfoParser.Parse(sfaInfo) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    ::optiling::int8_sfa::SFATilingCheck tilingChecker(sfaInfo);
    if (tilingChecker.Process() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    ::optiling::int8_sfa::SFAMlaTiling tiling(context);
    return tiling.DoOpTiling(&sfaInfo);
}

ge::graphStatus TilingPrepareForInt8SparseFlashAttention(gert::TilingParseContext* const context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(Int8SparseFlashAttention)
    .Tiling(TilingInt8SparseFlashAttention)
    .TilingParse<Int8SparseFlashAttentionCompileInfo>(TilingPrepareForInt8SparseFlashAttention);
} // namespace optiling
